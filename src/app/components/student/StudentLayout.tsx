...
import {
  LayoutDashboard, FileText, Brain, Bookmark, User, LogOut, Bell, Menu, X, ChevronRight
} from "lucide-react";
      ...
      <aside className="hidden lg:flex flex-col w-60 bg-[#1E3A8A] fixed h-full z-30">
        <div className="p-5 border-b border-blue-700">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-bold text-base" style={{ fontFamily: "Poppins, sans-serif" }}>MahaBoard</span>
          </div>
        </div>
            ...
            {navItems.map(item => (
              <button
                ...
                className={`flex-1 flex flex-col items-center justify-center py-3 px-2 transition-all text-xs ${
                  view === item.view
                    ? "text-[#1E3A8A] border-t-2 border-[#1E3A8A] bg-blue-50"
                    : "text-gray-500 hover:text-gray-700"
                }`}
              >
                <item.icon size={22} />
                <span className="mt-1 font-medium">{item.label.split(" ")[0]}</span>
              </button>
            ))}
          </nav>
        </div>
